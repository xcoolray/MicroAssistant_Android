...
(function (lng,angular) {
    ...
    function init() {
        var perspectiveWrapper = lng.dom(document.body),
            contentWrapper = container = lng.dom('#contentWraper');
        ...
        angular.showMenu = function (ev, target) {
            ...
            contentWrapper.style('width', target.width() + 'px');
            contentWrapper.style('height', target.height() + 'px');
            ...
            container[0].removeEventListener(clickevent, angular.hideMenu);
            container[0].addEventListener(clickevent, angular.hideMenu);
        };
        //showMenu.addEventListener(clickevent, angular.showMenu);

        angular.hideMenu = function (ev) {
            ...
            if (perspectiveWrapper.hasClass('animate')) {
                var onEndTransFn = function (ev) {
                    ...
                    perspectiveWrapper.append(contentWrapper.children());
                };
                if (support) {
                    perspectiveWrapper[0].addEventListener(transEndEventName, onEndTransFn);
                }
                else {
                    onEndTransFn.call();
                }
